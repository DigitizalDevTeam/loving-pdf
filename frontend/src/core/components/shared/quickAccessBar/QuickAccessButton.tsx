...
const QuickAccessButton: React.FC<QuickAccessButtonProps> = ({
  ...
  const bgColor = 'var(--icon-inactive-bg)';
  const textColor = 'var(--icon-inactive-color)';
  ...
  return (
    <div className="flex flex-col items-center gap-1 p-2" data-tour={dataTour} style={{ backgroundColor: isActive ? '#c9cacb' : 'inherit'}}>
      <ActionIcon
        {...actionIconProps}
        size={buttonSize}
        variant="subtle"
